...
def load_patterns():
    if os.path.exists(PATTERN_FILE):
        with open(PATTERN_FILE) as f:
            return json.load(f)
    # Default patterns – replace with your own
    return [
...
# -------------------- GOOGLE SHEETS SETUP --------------------
# Use environment variable for credentials (recommended) or fallback to file.
...
def mutate_pattern(pattern):
    ...
    # Mutate prompt: insert random adjective
...
def evaluate_pattern(pattern, test_inputs=None):
    """Placeholder: in production, call an LLM to evaluate. For now, return random score."""
