...
async def chat(request: dict):
    ...
    context = "You are LROS, a professional, serious AI assistant for business and medical topics.\n"
    if domain == "medical":
        med_muts = db.table("mutations").select("content").ilike("content", "%cancer%").limit(5).execute()
        context += "Relevant medical insights:\n" + "\n".join([m["content"] for m in med_muts.data]) + "\n"
    elif domain == "business":
        bus_muts = db.table("mutations").select("content").ilike("content", "%marketing%").limit(5).execute()
    ...
    db.table("agent_messages").insert({
        "agent_id": "chat_user",
        "message": f"[CHAT] User asked: {message}\nLROS answered: {response}",
        "round": 0,
        "sent_at": datetime.utcnow().isoformat(),
        "processed": False
    }).execute()
